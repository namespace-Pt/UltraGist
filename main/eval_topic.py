...
class Args(ModelArgs):
    eval_data: str = field(
        default="ultragist:longeval/topic_retrieval.json",
    ...
    adapt_window: bool = field(
        default=False,
        metadata={'help': 'Dynamically change the ultragist window so that the input is always compressed?'}
    )
...
def main():
    ...
    if args.adapt_window:
        ultragist_window = getattr(model.config, "ultragist_window", None)
        ...
        for i, x in enumerate(tqdm(dataloader, desc=f"Evaluating {num} Topics")):
            ...
            if hasattr(model, "memory"):
                if args.adapt_window:
                    ...
                    if length < ultragist_window:
                        ultragist_window = (length // 256) * 256
                        ultragist_stride = ultragist_window
                        model.memory.set(
                            ultragist_window=ultragist_window,
                            ultragist_stride=ultragist_stride,
                        )
